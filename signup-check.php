...
if (isset($_POST['uname'])  && isset($_POST['email']) && isset($_POST['number']) && isset($_POST['password'] )
	...
	if (empty($email)) {
        ...
        header("Location: signup.php?error= Number is required&$numb");
        exit();
    }
		...
		header("Location: signup.php?error=Email is not valid&$user_data");
		exit();
	  }


	if(strlen($pass) < 8 ) {
	header("Location: signup.php?error=Password must be at least 8 characters in length.&$user_data");
	exit();
	}

	if(strlen($pass) < 8 || !$uppercase ) {
		header("Location: signup.php?error=Password one upper case letter.&$user_data");
		exit();
		}
		else if(strlen($pass) < 8 || !$lowercase ) {
			header("Location: signup.php?error=Password must be one lower case letter.&$user_data");
			exit();
			}
